...
def live_collect_data(request):
    ...
    matches = read_csv_to_dict('matches.csv')  # Match bilgilerini tutan CSV dosyası
    home_players = read_csv_to_dict('home_players.csv')  # Home oyuncularını tutan CSV dosyası
    away_players = read_csv_to_dict('away_players.csv')  # Away oyuncularını tutan CSV dosyası
    # Verileri CSV'den al
    merged_data = read_csv_files()
    # scrape_matches(selected_league, selected_season, formatted_date, 1)
    # CSV dosyalarını yükleme
    # match_data = read_csv("data_general.csv")
    # home_players = read_csv("data_home.csv")
    # away_players = read_csv("data_away.csv")
    # merged_data = []
    # home_data = []
    # away_data = []
    ...
    #     match_id = match[0]
    #     home_data = [player for player in home_players if player[0] == match_id]
    #     away_data = [player for player in away_players if player[0] == match_id]
    #     merged_data.append({
    #         "match": match,
