...
def live_collect_data(request):
    ...
    for match_data in match_data:
        for home_player in home_players:
            if home_player[0] == match_data[0][0]:
                home_data.append(home_player)
        for away_player in away_players:
            if away_player[0] == match_data[0][0]:
                away_data.append(away_player)        
        merged_data.append({
            "match": match_data,
            "home_players": list(home_data),
            "away_players": list(away_data),
        })
    print(merged_data)
