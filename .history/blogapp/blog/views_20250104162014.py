...
def live_collect_data(request):
    ...
    match_data = []
    with open('data_general.csv', newline='', encoding='utf-8') as csvfile:
    ...
    # Futbolcu verileri
    home_players  = []
    with open('data_home.csv', newline='', encoding='utf-8') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            home_players .append(row)

    away_players = []
    with open('away_players.csv', newline='', encoding='utf-8') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
    ...
    for match in match_data:
        row = {key: match[key] for key in match}  # Maç bilgileri
        # Ev sahibi futbolcuları ekle
        row.update({
            f"Home_Player_{player['ID']}": player['Name']
            for player in home_players if player['ID'] in match.values()
        })
        # Deplasman futbolcuları ekle
        row.update({
            f"Away_Player_{player['ID']}": player['Name']
            for player in away_players if player['ID'] in match.values()
        })
        combined_data.append(row)
    
    # Dinamik sütunları belirle
    home_columns = [f"Home_Player_{player['ID']}" for player in home_players]
    away_columns = [f"Away_Player_{player['ID']}" for player in away_players]
