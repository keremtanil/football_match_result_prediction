...
def scrape_matches(selected_league, selected_season, formatted_date, match_id, output_file_general):
...
def live_collect_data(request):
    ...
    matches = []  # Veri çekildikçe bu listeyi dolduracağız
    try:
        # Selenium ile veri çekme işlemleri
        match_id = 1  # ID'yi başlatıyoruz (veya mevcut ID'yi alabilirsiniz)
        output_file_general = 'output.csv'  # Çıktı dosyası
        scrape_matches(selected_league, selected_season, selected_date, match_id, output_file_general)
        
        # CSV dosyasındaki verileri okumak ve matches listesine aktarmak
        with open(output_file_general, mode="r", encoding="utf-8") as file:
            reader = csv.reader(file)
            for row in reader:
                if row[0] != "ID":  # Başlık satırını atla
                    matches.append({
                        "league": row[1],
                        "season": row[2],
                        "match_date": row[3],
    ...
    context = {
        ...
        "seasons": data["seasons"],
        "form": form,
        "matches": matches,  # Bu veriyi template'ye gönderiyoruz
    }
