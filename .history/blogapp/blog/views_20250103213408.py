...
def scrape_matches(selected_league, selected_season, formatted_date, match_id):
        ...
        for row in rows:
            ...
            if row_date.date() == formatted_date.date():
                ...
                if row_data:
                    row_data.pop()
                    #first table
                    genel_data.append([genel_id] + row_data)
                    genel_id += 1
                ...
                for row in home_rows[:len(home_rows)-1]:  # Başlık satırını atla
                    ...
                    #second table
                    home_data.append =([match_id] + row_data)   # Her oyuncu için ayrı satır
                        ...
                        for row in away_rows[:len(home_rows)-1]:
                            ...
                            for cell in cells:
                                ...
                                if cell_text == "":  # Hücre boşsa
                                    ...
                                    row_data.append(cell_text)
                            #third table
                            away_data.append = ([match_id] + row_data)  # Her oyuncu için ayrı satır
                        ...
                        for row in away_rows[:len(home_rows)-1]:  
                            ...
                            for cell in cells:
                                ...
                                if cell_text == "":  # Hücre boşsa
                                    ...
                                    row_data.append(cell_text)
                            #third table
                            away_data = [match_id] + row_data  # Her oyuncu için ayrı satır
                driver.back()
                time.sleep(2)
                match_id += 1
    print(genel_data)
    ...
    driver.quit()
    return genel_data, home_data, away_data
def live_collect_data(request):
    ...
    genel_data, home_data, away_data = scrape_matches(selected_league,selected_season,formatted_date,1)


     # Şimdi bu verileri filtrele ve birleştir
    merged_data = []
    
    for genel, home, away in zip(genel_data, home_data, away_data):
        merged_data.append({
            "match": genel,
            "home_players": home,
            "away_players": away
    ...
    print("deneme")
    print(merged_data[0]['match'])
    context = {
        "leagues": data["leagues"],
        "seasons": data["seasons"],
        "form": form,
        "merged_data": merged_data,  # Birleştirilen veriler burada
    }
